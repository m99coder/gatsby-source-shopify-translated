...
const sourceNodes = async (
  ...
  const queries = { ...defaultQueries, ...shopifyQueries } // Convenience function to namespace console messages.
  ...
  try {
          ...
          async (product, productNode) => {
                ...
                async edge => {
                  ...
                  if (v.metafields)
                    await (0, _pIteration.forEach)(
                      ...
                      async edge =>
                        createNode(
                          await (0, _nodes.ProductVariantMetafieldNode)(
                            imageArgs
                          )(edge.node)
                        )
                    )
                  return createNode(
                    await (0, _nodes.ProductVariantNode)(
                      imageArgs,
                      productNode
                    )(edge.node)
                  )
            ...
            if (product.metafields)
              await (0, _pIteration.forEach)(
                ...
                async edge =>
                  createNode(
                    await (0, _nodes.ProductMetafieldNode)(imageArgs)(edge.node)
                  )
              )
            if (product.options)
              await (0, _pIteration.forEach)(product.options, async option =>
                createNode(
                  await (0, _nodes.ProductOptionNode)(imageArgs)(option)
                )
              )
          }
        ),
    ...
    if (includeCollections.includes(_constants.CONTENT)) {
      promises = promises.concat([
        createNodes(_constants.BLOG, queries.blogs, _nodes.BlogNode, args),
        createNodes(
          _constants.ARTICLE,
          queries.articles,
          _nodes.ArticleNode,
...
const createShopDetails = async ({
  ...
  // // Message printed when fetching is complete.
  const msg = formatMsg(
    `fetched and processed ${_constants.SHOP_DETAILS} nodes`
  )
  if (verbose) console.time(msg)
  await (0, _pIteration.forEach)(languages, async locale => {
    const { shop } = await (0, _lib.queryOnce)(
      createTranslatedClient(locale),
      queries.shopDetails
    )
